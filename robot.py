...
class Robot:
    ...
    def __init__(self, name="ASM_CAESAR", analytical_derivatives=False):
        """Start the Robot.
    ...
    def load_from_json(self, analytical_derivatives):
       ...
       ##### Modified #####
        #self.file_path=os.path.join(self.script_dir,'../models/robots/'+self.name + '.json')
        #with open(self.file_path, 'r') as f:
        #with open("./models/robots/" + self.name + ".json", "r") as f:
        with open(self.ws_path+"/models/robots/" + self.name + ".json", "r") as f:
        ...
        ##### Modified #####
        if 'forward_dynamics_path' in json_dict:
            self.fd = Function.load(self.script_dir+str(json_dict['forward_dynamics_path']))
        if 'inverse_dynamics_path' in json_dict:
            self.id = Function.load(self.script_dir+str(json_dict['inverse_dynamics_path']))
        if 'forward_kinematics_path' in json_dict:
            self.fk = Function.load(self.script_dir+str(json_dict['forward_kinematics_path']))
        if 'mass_matrix_path' in json_dict:
            self.M = Function.load(self.script_dir+str(json_dict['mass_matrix_path']))
        if 'mass_inverse_matrix_path' in json_dict:
            self.Minv = Function.load(self.script_dir+str(json_dict['mass_inverse_matrix_path']))
        if 'coriolis_path' in json_dict:
            self.C = Function.load(self.script_dir+str(json_dict['coriolis_path']))
        if 'gravity_path' in json_dict:
            self.G = Function.load(self.script_dir+str(json_dict['gravity_path']))
        if 'Jacobian_forward_dynamics_path' in json_dict:
            self.J_fd = Function.load(self.script_dir+str(json_dict['Jacobian_forward_dynamics_path']))
        if 'Jacobian_inverse_dynamics_path' in json_dict:
            self.J_id = Function.load(self.script_dir+str(json_dict['Jacobian_inverse_dynamics_path']))
        ...
        """
############################################################################
        # rename the jacobians due to some error that Sunhong is having
        ...
        if analytical_derivatives:
            ...
            print("completed analytical_derivatives")
